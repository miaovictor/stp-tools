<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="deviceFilter.state" :placeholder="$t('device.state')" clearable style="width: 150px" class="filter-item" @change="handleFilter">
    ...
    <el-table v-loading="listLoading" :data="deviceList" border fit highlight-current-row style="width: 100%;">
      <el-table-column :label="$t('device.id')" prop="id" sortable="custom" align="center" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('device.state')" width="100px" align="center">
        <template slot-scope="scope">
          <svg-icon :icon-class="scope.row.state === 0 ? 'online' : 'offline'" class="state-icon"/>
        </template>
      </el-table-column>
      <el-table-column :formatter="deviceTypeFormatter" :label="$t('device.type')" prop="addtion.board_version" width="100px" align="center"/>
      <el-table-column :formatter="decodeBase64Formatter" :label="$t('device.name')" prop="device_name" min-width="200px" align="center"/>
      <el-table-column :label="$t('device.sn')" prop="sn" width="200px" align="center"/>
      <el-table-column :formatter="decodeBase64Formatter" :label="$t('device.area')" prop="addr_more_info" width="200px" align="center"/>
      <el-table-column :label="$t('device.version')" prop="addtion.version" width="200px" align="center"/>
      <el-table-column :label="$t('device.operate')" align="center" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleOpen(scope.row)">{{ $t('device.open') }}</el-button>
          <el-button type="danger" size="mini" @click="handleUnbind(scope.row)">{{ $t('device.unbind') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <pagination v-show="deviceTotal>0" :total="deviceTotal" :page.sync="page" :limit.sync="deviceQuery.limit" @pagination="getDeviceList"/>
  </div>
</template>
